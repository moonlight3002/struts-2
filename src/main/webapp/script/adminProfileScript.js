...
$(function(){
		...
		$(document).on('click','.deletePermanentBtn', function(){
			$elem = $(this)
			var id = $(this).data("id");
			var cat = $(this).data("category");
			deleteData($elem.data('action'), $elem.data("action-category"),$elem.data("id"), function(){
				$(`.updateForm[data-category=${cat}]`).find(`.update-data-item[data-id="${id}"]`).remove();
...
function getupdateData(action){
	...
	if (action=="updateEducation"){
		$(`form[data-action=${action}] > .update-data-item:not(.newItem)`).each(function(){
			...
			data['deleted'] = $(this).find('input[name="deleted"]').prop("checked");
	...
	datas['userID'] = userProfileID;
	datas['userName'] = userUserName;
	obj['user'] = datas
	console.log(obj);
	return obj;
}
...
function getDeleteData(action, id){
	...
	var datas={}
	var user = {}
	if(action=="deleteEducationPermanently"){
		datas['educationID'] = id;
		obj['education'] = datas
	}else if(action=="deleteExperiencePermanently"){
		datas['experienceID'] = id;
		obj['experience'] = datas
	}else if(action=="deleteSkillPermanently"){
		datas['skillID'] = id;
		obj['skill'] = datas
	}
...
function appendUpdateDataSuccess(action, datas){
	$.each(datas, function(i, item){
		if(datas[i].show == true && datas[i].deleted == false){
			console.log(datas[i])
	...
	if (action == "viewAllEducations"){
		...
		$.each(data, function(i, item){
			...
			if(data[i].deleted === true){
				$cloneItem.addClass("deleted-item");
			}
			...
			$cloneItem.find('input[name="deleted"]').prop("checked", JSON.parse(`${data[i].deleted}`));
			$cloneItem.find('.deletePermanentBtn').attr("data-id", `${data[i].educationID}`);
			$cloneItem.removeClass("newItem");
		...
		$.each(data, function(i, item){
			...
			$cloneItem.find('input[name="deleted"]').prop("checked", JSON.parse(`${data[i].deleted}`));
			$cloneItem.find('.deletePermanentBtn').attr("data-id", `${data[i].experienceID}`);
			$cloneItem.removeClass("newItem");
		...
		$.each(data, function(i, item){
			...
			$cloneItem.find('input[name="show"]').prop("checked", JSON.parse(`${data[i].show}`));
			$cloneItem.find('input[name="deleted"]').prop("checked", JSON.parse(`${data[i].deleted}`));
			$cloneItem.find('.deletePermanentBtn').attr("data-id", `${data[i].skillID}`);
			$cloneItem.removeClass("newItem");
...
function getUserData(action){
	$('.update-data-item:not(.newItem)').remove();
	 $.ajax({
	        ...
	        url: `${action}.action`,
	        data: JSON.stringify({
	        	'user': {'userID' : userProfileID, 'userName' : userUserName} 
	        }),
...
function uploadPicture(action){
		    ...
		    file.append('userID', userProfileID);
		    console.log("file",file);
		    $.ajax({
		        type: "POST",
		        enctype: 'multipart/form-data',
		        url: `${action}.action?user.userID=${userProfileID}`,
		        data : file,
		        contentType: false,
		        processData: false,
		        success: function(data) {
		            ...
		            if (action == "uploadProfilePicture"){
		            	...
		            	if(data.headerPic!=null){
		            		$('.profile-Header-User-headerPicture').css("background-image", "url(getFile?userID="+userProfileID+"&profilePic="+data.headerPic+")");
